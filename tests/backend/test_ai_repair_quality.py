...
class TestSuccessfulPatchFlow:
    ...
    async def test_patch_and_probe_pass(self, tmp_path):
        ...
        provenance = "// =====\n// Generated by HIPForge\n// =====\n\n"
        source = SAMPLE_SOURCE + "void transfer(float* d, float* s, int n, hipStream_t st) {\n    hipMemcpyAsync_WRONG(d, s, n*sizeof(float), hipMemcpyDeviceToDevice, st);\n}\n"
        _orig = provenance + source
        fixed = source.replace("hipMemcpyAsync_WRONG", "hipMemcpyAsync")
        (tmp_path / "generated" / "kernel.hip").write_text(_orig)
        ...
        assert "Generated by HIPForge" not in ctx.patch_validation["diff"]
        assert ctx.patch_validation["changed_lines"] == 2
